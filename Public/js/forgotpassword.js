...
document.addEventListener("DOMContentLoaded", function () {
  ...
  forgotOtpForm.addEventListener("submit", function (event) {
    ...
    fetch("/forgot-password/send-otp", {
      ...
      .then((data) => {
        if (data.success) {
          startTimer(60, timerDisplay);
          document.getElementById("forgot-otp").classList.remove("active");
  ...
  verifyOtpForm.addEventListener("submit", function (event) {
    ...
    fetch("/forgot-password/verify-otp", {
      ...
      .then((data) => {
        if (data.success) {
          document.getElementById("verify-otp").classList.remove("active");
          document.getElementById("set-new-password").classList.add("active");
        } else {
          alert(data.message);
        }
      })
  ...
  resendButton.addEventListener("click", function (event) {
    ...
    fetch("/forgot-password/send-otp", {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        yourRole: document.querySelector(
          '#forgot-otp-form select[name="yourRole"]'
        ).value,
        email: document
          .querySelector('#forgot-otp-form input[name="email"]')
