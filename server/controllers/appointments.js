...
exports.bookAppointment = (req, res) => {
  ...
  workerT = removeSpaces(workerType);
  ...
  connection.query(
    query,
    [workerId, userId, workerT, taskDetails, taskDate, taskTime],
    (err, results) => {
      if (err) {
...
exports.getWorkerRequests = (req, res) => {
  ...
  workerT = removeSpaces(profession);
  function removeSpaces(str) {
    return str.replace(/\s+/g, '');
  }
  const query = `SELECT * FROM appointments WHERE worker_id = ? AND worker_type = ? `;

  connection.query(query, [workerId, workerT], (err, appointments) => {
    if (err) {
