...
exports.sendOTP = (req, res) => {
  ...
  let tableName;
  if (yourRole === "customer") {
    tableName = "users";
  } else if (yourRole === "guide") {
    tableName = "guide";
  } else if (yourRole === "taxi-driver") {
    tableName = "taxidriver";
  } else {
    return res
      .status(400)
  ...
  db.query(
    ...
    (err, result) => {
      ...
      db.query(
        "INSERT INTO otp_storage (otp, email) VALUES (?, ?)",
        [otp, email],
        (err, result) => {
          if (err) {
          ...
          const transporter = nodemailer.createTransport({
            service: "gmail",
            auth: {
              user: "guidewheels24@gmail.com",
              pass: "pborejswuqaqxapj",
            },
          });
...
exports.verifyOTP = (req, res) => {
  ...
  db.query("SELECT * FROM otp_storage WHERE otp = ?", [otp], (err, result) => {
    if (err) {
      console.error("Error querying OTP storage:", err);
      return res
        .status(500)
        .json({ success: false, message: "Database query error." });
    }
...
// Function to set new password
exports.setPassword = (req, res) => {
  const { email, password, yourRole } = req.body;
